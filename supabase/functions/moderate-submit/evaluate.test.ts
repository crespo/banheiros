...
test("rejects content when a score exactly equals its threshold", () => {
  const scores = { TOXICITY: 0.8, PROFANITY: 0.1, THREAT: 0.1, IDENTITY_ATTACK: 0.1 };
  const thresholds = { TOXICITY: 0.8, PROFANITY: 0.8, THREAT: 0.8, IDENTITY_ATTACK: 0.8 };
  expect(evaluateModeration(scores, thresholds).verdict).toBe("rejected");
});
