...
### Design tokens
- `src/styles/tokens.css`: CSS variables com os valores do design system "Organic" do handoff (`docs/design/_ds/.../styles.css` + overrides de `docs/design/app.css`): cores + ramps, espaçamento (4.4 a 35.2px), radius (sm/md/lg + pill), sombras, tipografia.
...
### i18n
- Módulo próprio mínimo em `src/i18n/`: dicionários `pt.ts` e `en.ts` migrados verbatim das chaves do handoff (`docs/design/js/i18n.js`, sem a lista `banned`, que morreu com a ADR-0005), função `t(key, vars)` com interpolação `{{var}}`, idioma persistido em localStorage até existir perfil.
...
- `t('auth.welcomeTitle')` retorna a string do idioma ativo; trocar idioma reflete imediatamente e sobrevive a reload.
- CI verde no push.
