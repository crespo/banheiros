...
export default function MapScreen({ bathrooms = [] }: { bathrooms?: Bathroom[] }) {
  ...
  return (
    <>
      ...
      {bathrooms.map(b => { const category = categorizeBathroom(b.kind, b.paid); return <button key={b.id}><Icon name={category.icon} />{bathroomDisplayName(b.name, t("bathroom.unnamed"))}{b.paid && <Icon name="dollarSign" />}</button>; })}
    </>
  );
}
